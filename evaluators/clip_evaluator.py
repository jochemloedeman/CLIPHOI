...
class CLIPEvaluator(object):
    ...
    def __init__(
        ...
        """
        Class that is intended for evaluating OpenAI's CLIP
        (https://github.com/openai/CLIP) on a PyTorch Dataset,
        ...
        self.prob_fn_matcher = {
            ...
            "prob_based_greater": functools.partial(
                prob_based_tensor,
                mode="prob_based_greater",
                threshold=self.ir_threshold),
            "prob_based_smaller": functools.partial(
                prob_based_tensor, mode="smaller",
                threshold=self.ir_threshold),
            "logit_based": functools.partial(
                logit_based_tensor,
                threshold=self.ir_threshold),
            "ir_softmax": None
        }
    ...
    def _evaluate_map(self) -> None:
        """Iterates through a DataLoader that is created from the given
        Dataset. In each iteration, the images are passed through the CLIP
        image encoder. The resulting embeddings are then compared to the HOI
        class embeddings to obtain logits per image. A distribution over the
        HOI classes is computed using a softmax.
        """
        ...
        text_inputs = torch.cat(
            [clip.tokenize(f"a person {hoi.hoi_phrase}") for hoi in
             self.dataset.hoi_classes]
        ).to(self.device)
        ...
        for images, target in tqdm(dataloader):
            ...
            if self.five_crop:
                ...
                image_features = self.model.encode_image(
                    images.to(self.device).view(-1, channels, height, width))
                image_features = image_features.view(batch_size, nr_of_crops,
                                                     -1).mean(1)
            else:
        ...
        logit_statistics = {
            position: logit_statistics[position] / len(self.dataset) for
            position in
...
def to_probabilities_by_threshold(logits, prob_threshold):
    ...
    new_probabilities = torch.gather(logits, dim=-1,
                                     index=sorted_indices).softmax(dim=-1)
    ...
    for batch_index in range(batch_size):
        ...
        if len(candidate_positives) > 1:
            for class_index in candidate_positives:
                included_indices = torch.cat(
                    (candidate_negatives, class_index.unsqueeze(dim=0)))
                increased_probability = torch.max(
                    sample_logits[included_indices].softmax(dim=-1))
...
def custom_collate(batch):
    batch = [(image, image_tensor.unsqueeze(0), target.unsqueeze(0)) for
             image, image_tensor, target in batch]
    return batch
...
if __name__ == '__main__':
    ...
    patch_size = 16
    interpolated_pos_embed = interpolate_pos_encoding(random_image_embeds, w, h,
                                                      pos_embed, patch_size)
    print()
