...
class HICOmAP(Metric):
    """
    Subclass of the TorchMetrics base class. Implements mean average
    precision, as intended by the creators/authors of the HICO dataset (
    ...
    Args:
        ...
        known_object_mode:
            ...
            Only images annotated as negatives for a how are taken into
            account in the mAP calculation
    """

    def __init__(self, hico_dataset: HICODataset,
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor) -> None:
        ...
        For each update, indexes of equal shapes are added that identify the
        preds and targets to the corresponding HOI classes
        """
        assert preds.shape == target.shape

        self.indexes.append(
            torch.arange(preds.shape[1]).repeat(preds.shape[0], 1))
        self.preds.append(preds)
        self.target.append(target)

    def compute(self) -> torch.Tensor:
        """Computes the mean average precision from a Tensor containing the
        AP's per HOI """
    ...
    def _compute_ap_per_hoi(self) -> torch.Tensor:
        """First flatten the accumulated preds, targets and indexes. Then
        determine the image-hoi pairs that should be included in the AP
        calculations and calculate the AP per HOI-group.
        """
        ...
        indexes[included_pairs], preds[included_pairs], target[included_pairs])
    ...
    def _get_included_pairs(self, target: torch.Tensor) -> Tuple[torch.Tensor]:
        """Calculate indices of included image-hoi pairs based on which
        evaluation mode is chosen """
    ...
    def _get_ap_per_hoi(
    ...
    ) -> Union[Dict[str, float], Dict[str, Tuple[float, int]]]:
        ...
        if include_statistics:
            ...
            ap_per_hoi = {
                hoi: (ap.item(), statistics[hoi])
                for hoi, ap in ap_per_hoi.items()
            }
    ...
    def export_to_json(self, filename: str) -> None:
        json_list = [{'map': self._mean_ap},
                     self._get_ap_per_hoi(include_statistics=True)]
